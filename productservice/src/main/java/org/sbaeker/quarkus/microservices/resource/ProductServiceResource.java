 ...
 * orders and provides
 * a stream of recipes. It receives orders as JSON and forwards them to the
 * appropriate queues based
 * on their classification. It also provides a server-sent event (SSE) stream of
 * recipes.
 *
 * <p>
 * Usage example:
 ...
 *
 * <p>
 * The ProductServiceResource class is annotated with {@code @Path} to specify
 * the base path for
 * the resource.
 *
 * <p>
 * The resource has the following endpoints:
 *
 * <ul>
 * <li>{@code POST /product-service}: Receives orders as JSON and forwards them
 * to the appropriate
 * queues.
 * <li>{@code GET /product-service/recipes}: Provides a server-sent event (SSE)
 * stream of recipes.
 * </ul>
 *
 * <p>
 * The resource uses CDI (Contexts and Dependency Injection) to inject the
 * necessary
 * dependencies, such as emitters and the classifier.
 *
 * <p>
 * Note: This class assumes that there are predefined channels named
 * "barista-in" and
...
public class ProductServiceResource {
    ...
    /**
     * Handles incoming orders by forwarding them to the appropriate queues based on
     * their
     * classification.
     *
