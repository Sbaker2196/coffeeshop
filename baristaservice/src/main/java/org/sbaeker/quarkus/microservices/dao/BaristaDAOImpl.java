...
public class BaristaDAOImpl implements BaristaDAO {
  ...
  BaristaDAOImpl(MeterRegistry registry) { this.registry = registry; }
    ...
    try {
      Recipe recipe =
          entityManager
              ...
              .getSingleResult();
      LOG.info("Recipe successfully retrieved from BaristaRecipeDB: " +
               recipe.toString());
      return recipe;
    } catch (NoResultException e) {
      registry.counter("barista.service.amount.of.failed.db.retrievals")
          .increment();
  ...
  public List<Recipe> getAllRecipesFromDB() {
    ...
    try {
      ...
      LOG.info(
          "Orders have been successfully retrieven from the BaristaRecipeDB");
    } catch (Exception e) {
