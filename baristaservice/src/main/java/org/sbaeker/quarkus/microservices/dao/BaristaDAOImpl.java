...
/**
 * An implementation of the BaristaDAO interface that interacts with the BaristaRecipeDB to retrieve
 * recipe information.
 *
 * <p>The BaristaDAOImpl class provides methods to retrieve recipe details from the database,
...
public class BaristaDAOImpl implements BaristaDAO {
   ...
   *
   * <p>This method searches for a recipe in the database with a matching name. If found, it returns
   * the Recipe object representing the recipe. If no recipe is found, it returns null.
   *
   * @param name the name of the recipe to retrieve
   * @return the Recipe object representing the retrieved recipe, or null if the recipe is not found
   * @throws IllegalArgumentException if the name parameter is null or empty
   * @throws javax.persistence.PersistenceException if an error occurs during the database operation
   * @see Recipe
   */
  @Override
  @Transactional
  ...
  public Recipe retrieveRecipeFromDB(String name) {
    ...
    try {
      ...
      registry.counter("barista.service.amount.of.failed.db.retrievals").increment();
